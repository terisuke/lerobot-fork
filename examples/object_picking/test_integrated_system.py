...
from lerobot.control.realtime_controller import (
    ControlCommand,
    ControlState,
    RealTimeController,
)
...
class IntegratedSystemTester:
    ...
    def test_camera_system(self) -> bool:
        ...
        try:
            ...
            print(
                f"✅ Camera test passed: RGB {rgb_image.shape}, Depth {depth_image.shape}"
            )
            return True
    ...
    def test_object_detection(self) -> bool:
        ...
        try:
            ...
            print(
                f"✅ Object detection test passed: {len(detections)} objects detected"
            )
    ...
    def test_object_tracking(self) -> bool:
        ...
        try:
            ...
            for frame_num in range(30):  # 30 frames
                ...
                if frame_num % 10 == 0:
                    print(
                        f"  Frame {frame_num}: {len(tracked_objects)} objects tracked"
                    )
            ...
            print(
                f"✅ Object tracking test passed: {len(tracked_objects)} objects tracked"
            )
            return True
    ...
    def test_robot_control(self) -> bool:
        ...
        try:
            ...
            test_positions = [(0.1, 0.0, 0.2), (0.0, 0.1, 0.2), (0.0, 0.0, 0.3)]
    ...
    def test_integrated_system(self) -> bool:
        ...
        try:
            ...
            while time.time() - start_time < 10.0:
                ...
                print(
                    f"  State: {status.state.value}, Target: {status.current_target is not None}"
                )

                # Display camera feed
                try:
                    rgb_image, depth_image, intrinsics = (
                        self.detector.get_camera_frames()
                    )
                    detections = self.detector.detect_objects(
                        rgb_image, depth_image, intrinsics
                    )
                    ...
                    for obj in tracked_objects:
                        ...
                        color = (
                            (0, 255, 0) if obj == status.current_target else (255, 0, 0)
                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test Integrated Object Picking System"
    )
    parser.add_argument(
