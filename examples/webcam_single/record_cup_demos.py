...
class CupDemonstrationRecorder:
    ...
    def get_robot_state(self):
        ...
        if self.robot and self.robot.is_connected:
            try:
                ...
                return {
                    "joint_positions": obs.get("state", [0.0] * 6).tolist()
                    if hasattr(obs.get("state", []), "tolist")
                    else obs.get("state", [0.0] * 6),
    ...
    def visualize(self, frame, cup_detection, recording_status):
        ...
        cv2.putText(
            annotated, f"Robot: {robot_status}", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.6, robot_color, 2
        )

        # Camera flip status
        if self.camera_flip:
            cv2.putText(annotated, "Flip: ON", (10, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
        ...
        cv2.putText(
            annotated,
            "r: Record, s: Save, q: Quit | Arrow: Move, Space: Gripper",
...
def main():
    ...
    try:
        recorder = CupDemonstrationRecorder(
            output_dir=output_dir, camera_flip=camera_flip, enable_robot=enable_robot
        )
        recorder.run()
