...
class RealTimeController:
    ...
    def __init__(
        ...
        self.safety_limits = {"x": (-0.5, 0.5), "y": (-0.5, 0.5), "z": (0.0, 0.5)}
    ...
    def _update_tracking_state(self, tracked_objects: List[DepthTrackedObject]):
        ...
        if (
            current_target.depth_confidence >= self.tracking_confidence
            and current_target.age >= 10
    ...
    def _select_best_object(
        self, objects: List[DepthTrackedObject]
    ) -> DepthTrackedObject:
    ...
    def _calculate_safe_position(
        self, target_pos: Tuple[float, float, float]
