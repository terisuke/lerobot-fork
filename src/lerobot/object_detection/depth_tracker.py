...
class DepthObjectTracker:
    ...
    def __init__(
        ...
        size_change_threshold: float = 0.2,
    ):  # 20% size change
        """
    ...
    def _calculate_enhanced_distance_matrix(
    ...
    def _find_best_matches_with_depth(
        self, distances: np.ndarray, detections: List[DetectedObject]
    ) -> List[Tuple[int, int]]:
        """Find best matches considering depth consistency."""
        if distances.size == 0:
            return []
        ...
        for flat_idx in flat_indices:
            ...
            if (
                track_idx not in [pair[0] for pair in matched_pairs]
                and det_idx not in used_detections
                and distances[track_idx, det_idx] < self.max_distance
            ):
    ...
    def _validate_depth_consistency(
        self, track: DepthTrackedObject, detection: DetectedObject
    ) -> bool:
    ...
    def _calculate_depth_confidence(self, track: DepthTrackedObject) -> float:
        ...
        confidence = max(
            0.0, 1.0 - depth_variance / (self.depth_consistency_threshold**2)
        )
        return confidence
    ...
    def get_objects_in_depth_range(
        self, min_depth: float, max_depth: float
    ) -> List[DepthTrackedObject]:
    ...
    def get_objects_with_high_confidence(
    ...
    def predict_object_position(
        self, object_id: int, frames_ahead: int = 1
    ...
    def is_object_moving(
        self, object_id: int, velocity_threshold: float = 0.01
    ) -> bool:
    ...
    def get_stable_objects(
        self, min_age: int = 5, min_confidence: float = 0.7
    ) -> List[DepthTrackedObject]:
        ...
        for track in self.tracked_objects.values():
            if (
                track.age >= min_age
                and track.depth_confidence >= min_confidence
