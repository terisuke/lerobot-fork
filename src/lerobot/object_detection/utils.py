...
class DepthProcessor:
    ...
    @staticmethod
    def get_depth_in_region(depth_image: np.ndarray, 
    ...
    @staticmethod
    def create_depth_mask(depth_image: np.ndarray, 
                         min_depth: float = 0.1, 
                         max_depth: float = 2.0) -> np.ndarray:
        """
    ...
    @staticmethod
    def depth_to_world_coords(x: float, y: float, depth: float, 
                            intrinsics: rs.intrinsics) -> Tuple[float, float, float]:
        """
    ...
    @staticmethod
    def world_to_depth_coords(world_x: float, world_y: float, world_z: float,
                            intrinsics: rs.intrinsics) -> Tuple[float, float, float]:
        """
...
class ColorProcessor:
    ...
    @staticmethod
    def apply_color_filter(image: np.ndarray, 
    ...
    @staticmethod
    def find_color_objects(image: np.ndarray, 
...
class ObjectFilter:
    ...
    @staticmethod
    def filter_by_depth(objects: List, min_depth: float = 0.1, 
                       max_depth: float = 2.0) -> List:
        """
    ...
    @staticmethod
    def filter_by_size(objects: List, min_width: int = 20, 
                      min_height: int = 20) -> List:
        """
...
class VisualizationUtils:
    ...
    @staticmethod
    def draw_detection(image: np.ndarray, detection, 
    ...
    @staticmethod
    def draw_tracking_info(image: np.ndarray, tracked_object,
        ...
        if hasattr(tracked_object, 'object_id'):
            x, y, w, h = detection.bbox
            cv2.putText(image, f"ID: {tracked_object.object_id}", 
                       (x, y + h + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, thickness)
        
        # Add velocity info
        if hasattr(tracked_object, 'velocity'):
            vx, vy, vz = tracked_object.velocity
            speed = np.sqrt(vx**2 + vy**2 + vz**2)
            cv2.putText(image, f"Speed: {speed:.3f} m/s", 
