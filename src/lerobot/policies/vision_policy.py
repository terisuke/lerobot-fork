...
class VisionEncoder(nn.Module):
    ...
    def __init__(
        self,
        input_channels: int = 4,  # RGB + Depth
        hidden_dim: int = 256,
        output_dim: int = 512,
    ):
        super().__init__()
    ...
    def forward(
        self, rgb_image: torch.Tensor, depth_image: torch.Tensor
    ) -> torch.Tensor:
...
class ObjectEncoder(nn.Module):
    ...
    def __init__(
        self,
        object_dim: int = 10,  # bbox + center + depth + confidence + class
        hidden_dim: int = 128,
        output_dim: int = 256,
    ):
        super().__init__()
...
class VisionPolicy(nn.Module):
    ...
    def __init__(
        ...
        self.robot_encoder = nn.Sequential(
            nn.Linear(6, 64), nn.ReLU(), nn.Linear(64, 128)  # 6DOF robot state
        )
    ...
    def forward(
        ...
        combined_features = torch.cat(
            [vision_features, attended_objects, robot_features], dim=1
        )
    ...
    def _find_target_object(
        self, tracked_objects: List[DepthTrackedObject]
    ) -> Optional[int]:
    ...
    def _predict_trajectory(
        self, tracked_objects: List[DepthTrackedObject], target_object_id: Optional[int]
    ) -> Optional[np.ndarray]:
